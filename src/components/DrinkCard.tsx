...
export default function DrinkCard({ drink, editMode, onAddToCart, onEdit, onDelete, onSetStock }: DrinkCardProps) {
      ...
      <div className="min-w-0">
        ...
        {(!drink.is_available || outOfStock) && (
          <span className="inline-block mt-1 text-[10px] font-semibold uppercase tracking-wide text-red-600 bg-red-50 px-2 py-0.5 rounded-full">
            {outOfStock && drink.is_available ? 'Sin stock' : 'No disponible'}
          </span>
        )}
      </div>
      ...
      {editMode ? (
        <div className="flex flex-col gap-2 shrink-0 items-end">
          <div className="flex items-center gap-1.5">
            <span className="text-[10px] font-semibold uppercase tracking-wide text-zinc-400">Stock</span>
            <button
              onClick={() => onSetStock(drink, Math.max(0, drink.stock - 1))}
              aria-label={`Bajar stock de ${drink.name}`}
              className="w-6 h-6 rounded-full bg-zinc-100 text-zinc-600 hover:bg-zinc-200 flex items-center justify-center"
            >
              <Minus className="w-3.5 h-3.5" />
            </button>
            <span className="w-6 text-center text-sm font-bold">{drink.stock}</span>
            <button
              onClick={() => onSetStock(drink, drink.stock + 1)}
              aria-label={`Subir stock de ${drink.name}`}
              className="w-6 h-6 rounded-full bg-zinc-100 text-zinc-600 hover:bg-zinc-200 flex items-center justify-center"
            >
        ...
        <button
          ...
          disabled={!drink.is_available || outOfStock}
